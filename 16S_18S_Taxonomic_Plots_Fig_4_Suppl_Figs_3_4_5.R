#16S and 18S taxonomic and functional plots
...
#Same plot without the legends
bubble_plot_18S_division+ font_size + theme(axis.text.x = element_text(angle = 270, vjust = 0.5, hjust=1)) + expand_limits(y = 20) + guides(colour = FALSE)

#This version of plot doesn't have the guide to make it wider
ggsave("bubble_plot_18S_division_no_guide.jpg", width = 297, height = 210, units = c("mm"))
ggsave("bubble_plot_18S_division_no_guide.pdf", width = 297, height = 210, units = c("mm"))

#Protist functional annotation####
...
#Then make the plots for the different groups within
#Protist Function Barplot####
ggplot(protist_melt, aes(x = new_pond, y = Abundance, fill=Nutrition))+geom_bar(stat="identity", width = 0.8) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + scale_x_discrete(limits=average_order) + scale_fill_manual(values = function_palette) + font_size + theme(axis.text.x = element_text(angle = 270, vjust = 0.5, hjust=1))

#Save your plot
ggsave("protist_function_barplot.jpg", width = 297, height = 210, units = c("mm"))
ggsave("protist_function_barplot.pdf", width = 297, height = 210, units = c("mm"))


#Plot a bubble plot of all functional groups####
#Make a bubble plot of the 15 most abundant phototrophic classes####
phototrophic = subset_taxa(ps_protist, Nutrition %in% c("phototrophic")) 
phototrophic #1289 ASVs
class.sum = tapply(taxa_sums(phototrophic ), tax_table(phototrophic )[, "Class"], sum, na.rm=TRUE)
top15_classes = names(sort(class.sum, TRUE))[1:15]
top15_classes
phototrophic = prune_taxa((tax_table(phototrophic)[, "Class"] %in% top15_classes), phototrophic)
phototrophic <- tax_glom(phototrophic, taxrank = "Class")
phototrophic_melt <- psmelt(phototrophic)

#Plot a bubble plot of the phototrophic groups, colour coordinated
phototrophic_plot <- ggplot(phototrophic_melt, aes(x=new_pond, y = fct_reorder(Class, Abundance))) + geom_point(aes(size = Abundance, colour = Kingdom), alpha = 0.8)+ scale_size_continuous(limits = c(0.00001, 1), range = c(1,25), breaks = c(0.10, 0.25, 0.50, 1))+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + scale_x_discrete(limits=average_order) + labs( x= "", y = "", size = "Relative Abundance (%)", fill = "") + theme(strip.background = element_blank())+scale_colour_manual(values = "#228833", guide = "none")
phototrophic_plot + font_size + theme(axis.text.x = element_text(angle = 270, vjust = 0.5, hjust=1))

#Save your plot
ggsave("phototrophic.jpg", width = 315, height = 297, units = c("mm"))
ggsave("phototrophic.pdf", width = 315, height = 297, units = c("mm"))
...
#Not happy with the plots for metazoa, fungi, or cyanos and proteos so going to change these back to 0.1% cutoffs
...
#ALTERNATIVES
#Cyanobacteria top 15
cyanobacteria <- subset_taxa(ps_16S_ave_RA, Phylum == "Cyanobacteria")
order.sum = tapply(taxa_sums(cyanobacteria), tax_table(cyanobacteria)[, "Family"], sum, na.rm=TRUE)
top15_order = names(sort(order.sum, TRUE))[1:15]
top15_order
cyanobacteria = prune_taxa((tax_table(cyanobacteria)[, "Family"] %in% top15_order), cyanobacteria)
cyanobacteria <- tax_glom(cyanobacteria, taxrank = "Family")
cyanobacteria_melt <- psmelt(cyanobacteria)


cyanobacteria_palette <- interp_colors(15, colors = figure_palette)

#Plot relative abundance of cyanobacteria
cyanobacteria_plot <- ggplot(cyanobacteria_melt, aes(x = new_pond, y = fct_reorder(Family, Abundance))) + geom_point(aes(size = Abundance, colour = fct_rev(fct_reorder(Family, Abundance)), alpha = 0.8))+ scale_size_continuous(limits = c(0.00001, 1), range = c(1,25), breaks = c(0.01, 0.05, 0.10, 0.20, 0.50, 1))+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + scale_colour_manual(values = cyanobacteria_palette, guide = "none")+ scale_x_discrete(limits=average_order) + labs( x= "", y = "", size = "Relative Abundance (%)", fill = "") + theme(strip.background = element_blank())
cyanobacteria_plot + font_size + theme(axis.text.x = element_text(angle = 270, vjust = 0.5, hjust=1)) + guides(alpha = FALSE)

#use ggsave to save to your wd as a pdf and jpeg
ggsave("cyanobacteria_plot.jpg", width = 297, height = 210, units = c("mm"))
ggsave("cyanobacteria_plot.pdf", width = 297, height = 210, units = c("mm"))

#Proteobacteria top 15
proteobacteria <- subset_taxa(ps_16S_ave_RA, Phylum == "Proteobacteria")
order.sum = tapply(taxa_sums(proteobacteria), tax_table(proteobacteria)[, "Order"], sum, na.rm=TRUE)
top15_order = names(sort(order.sum, TRUE))[1:15]
top15_order
proteobacteria = prune_taxa((tax_table(proteobacteria)[, "Order"] %in% top15_order), proteobacteria)
proteobacteria <- tax_glom(proteobacteria, taxrank = "Order")
proteobacteria_melt <- psmelt(proteobacteria)

proteobacteria_palette <- interp_colors(15, colors = figure_palette)

#Plot relative abundance of proteobacteria
proteobacteria_plot <- ggplot(proteobacteria_melt, aes(x = new_pond, y = fct_reorder(Order, Abundance))) + geom_point(aes(size = Abundance, alpha = 0.8, colour = fct_rev(fct_reorder(Order, Abundance))))+ scale_size_continuous(limits = c(0.00001, 1), range = c(1,25), breaks = c(0.01, 0.05, 0.10, 0.20, 0.50, 1))+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + scale_colour_manual(values = proteobacteria_palette, guide = "none")+ scale_x_discrete(limits=average_order) + labs( x= "", y = "", size = "Relative Abundance (%)", fill = "") + theme(strip.background = element_blank())
proteobacteria_plot + font_size + theme(axis.text.x = element_text(angle = 270, vjust = 0.5, hjust=1))+ guides(alpha = FALSE)

#use ggsave to save to your wd as a pdf and jpeg
ggsave("proteobacteria_plot.jpg", width = 297, height = 210, units = c("mm"))
ggsave("proteobacteria_plot.pdf", width = 297, height = 210, units = c("mm"))


#Finally, let's import our RA data from Excel of the functions averaged by region

func_RA_table <- read.table("C:/Users/pmh36/OneDrive - Natural History Museum/R/R_data/dada2/func_ra_table.txt", header = TRUE)
func_RA_table
